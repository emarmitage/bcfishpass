...
(
  ...
  and barrier_type in ('GRADIENT_30')
  union all
  select
...
barriers_filtered as (
  ...
  where watershed_group_code = any(
      array(
        ...
        -- where st is true - replace for where cct is true??
        where st is true
      )
  )
...
insert into bcfishpass.barriers_cct
(
    barriers_cct_id,
    barrier_type,
    barrier_name,
...
-- ('ALBN','ATLL','ATNA','BABL','BABR','BARR','BELA','BRKS','BULK','CAMB','CHIR','CHWK','CLAY','COMX','COWN','DRIR','FRAN','FRCN','GOLD','GRAI','HARR','HOLB','HOMA','INKR','ISKR','JERV','KAKC','KEEC','KHTZ','KINR','KISP','KITL','KITR','KLAR','KLIN','KLUM','KNIG','KSHR','KTSU','KUMR','KUSR','LBIR','LDEN','LFRA','LILL','LISR','LKEL','LNAR','LNIC','LRDO','LSKE','LSTR','MBNK','MESC','MORI','MORR','MSKE','MSTR','NAHR','NAKR','NASC','NASR','NBNK','NECL','NECR','NEVI','NIEL','NIMP','OWIK','PARK','PORI','SALM','SANJ','SETN','SEYM','SHER','SIML','SKGT','SPAT','SQAM','STIR','STUL','TAHR','TAHS','TAKL','TASR','TATR','TAYR','TOBA','TSAY','TSIT','TUTR','TUYR','UBIR','UDEN','UEUT','UNAR','UNRS','UNUR','USKE','UTRE','VICT','WORC','ZYMO')
